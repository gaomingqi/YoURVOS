...
class VideoEvaluator:
    ...
    def __call__(self, vid_name):
        ...
        if self.skip_first_and_last:
            ...
            frames = frames[1:-1]
        evaluator = Evaluator(name=f'{vid_name[0]}, {vid_name[1]}')
        for f in frames:
            try:
                mask_array = np.array(Image.open(path.join(vid_mask_path, f)))/255
                if path.exists(path.join(vid_gt_path, f)):
                    gt_array = np.array(Image.open(path.join(vid_gt_path, f)))
                else:

                    gt_array = np.zeros_like(mask_array)
...
def benchmark(gt_roots,
    ...
    for gt_root, mask_root in zip(gt_roots, mask_roots):
        ...
        samples = []
        for video in videos:
            video_path = os.path.join(mask_root, video)
            exps = sorted(os.listdir(video_path))
            for exp in exps:
                samples.append((video, exp))
    ...
    for i, mask_root in enumerate(mask_roots):
        ...
        for name, (iou, boundary_f) in object_metrics.items():
            for object_idx in iou.keys():
                ...
                jf = (j + f) / 2
                out_string += f'{name[0]:<{ml}}, {name[1]}, {object_idx:03}, {jf:>4.1f}, {j:>4.1f}, {f:>4.1f}\n'
