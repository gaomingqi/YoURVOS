...
class CrossModalFPNDecoder(nn.Module):
    ...
    def forward_features(self, features, text_features, poses, memory, nf):
        ...
        for idx, (mem, f, pos) in enumerate(zip(memory[::-1], features[1:][::-1], poses[1:][::-1])): # 32x -> 8x
            ...
            vision_features = rearrange(vision_features, '(b t) c h w -> (h w) (b t) c', b=b, t=t)
            vision_pos = rearrange(pos, '(b t) c h w -> (h w) (b t) c', b=b, t=t)
            ...
            cur_fpn = rearrange(cur_fpn, '(h w) (b t) c -> (b t) c h w', t=t, h=h, w=w)
...
class VisionLanguageBlock(nn.Module):
    ...
    def forward_post(self, tgt, memory, t, h, w,
        ...
        if self.sr_ratio > 1: # downsample
            q = rearrange(q, '(h w) (b t) c -> (b t) c h w', t=t, h=h, w=w)
            k = rearrange(k, '(h w) (b t) c -> (b t) c h w', t=t, h=h, w=w)
            v = rearrange(tgt, '(h w) (b t) c -> (b t) c h w', t=t, h=h, w=w)
            # downsample
            ...
            q = rearrange(q, '(b t) c h w -> (h w) (b t) c', t=t)
            k = rearrange(k, '(b t) c h w -> (h w) (b t) c', t=t)
            v = rearrange(v, '(b t) c h w -> (h w) (b t) c', t=t)
            # downsample mask
...
class Language2VisionFusionModule(nn.Module):
...
class Vision2LanguageFusionModule(nn.Module):
    def __init__(self, d_model, nhead, ffn_hidden=2048, dropout=0.0):
        super().__init__()
        self.multihead_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.norm1 = nn.LayerNorm(d_model)
        self.dropout1 = nn.Dropout(dropout)
    ...
    def forward(self, tgt, memory,
        ...
        tgt = self.norm2(tgt + self.dropout2(tgt_ffn))
