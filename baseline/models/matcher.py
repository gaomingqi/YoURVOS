...
def span_(spans, targets):
        ...
        b, n_q = spans.shape[0], spans.shape[2]
        res = torch.zeros((n_q, b)).to(spans.device)

        for bi in range(b):
            for qi in range(n_q):
                pred_start_prob = (spans[[bi], :, [qi], 0]).softmax(1)                            # pred -> prob (only consider foreground)
...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1,
                       cost_mask: float = 1, cost_dice: float = 1, cost_span: float = 1, cost_relevance: float = 1, num_classes: int = 1):
        """Creates the matcher
    ...
    def forward(self, outputs, targets):
        ...
        for i in range(bs): 
            ...
            # tgt_ids = targets[i]["labels"]     # mute due to single class inference
    ...
    def spatial_temporal_forward(self, outputs, targets):
        ...
        src_masks = outputs["pred_masks"]
        src_valids = outputs["pred_valids"]

        bs, _, nq, h, w = src_masks.shape
        nf = src_logits.shape[1]
        ns = src_boxes.shape[1]
        ...
        indices = []
        cost_span = span_(src_spans, targets)
        for i in range(bs): 
            out_prob = src_logits[i].sigmoid() 
            out_bbox = src_boxes[i]
            out_mask = src_masks[i]
            out_valid = torch.transpose(src_valids[i].squeeze(-1), 0, 1)    # n_q, n_f

            # tgt_ids = targets[i]["labels"]     # mute due to single class inference
            tgt_bbox = targets[i]["boxes"]     
            tgt_valid = targets[i]["valid"]
            tgt_span = targets[i]["span"]   
            tgt_mask = target_masks[i] 

            # class cost
            # we average the cost on valid (relevance) frames
            cost_class = []
            for t in range(nf):
                if tgt_valid[t] == 0:
                    continue
            ...
            # valid cost pred_valids, b x num_frames
            target_valid = torch.stack([tgt_valid], dim=0).float().repeat(nq, 1)  # 1, num_frames
            weight = torch.full(out_valid.shape, 0.1, device=out_valid.device)
            
            temp_bound = tgt_span
            weight[:,temp_bound[0]:(temp_bound[1]+1)] = 1
        
            cost_valid = F.binary_cross_entropy_with_logits(out_valid, \
                    target_valid, weight=weight, reduction='none')
            
            cost_valid = cost_valid.mean(1).unsqueeze(-1)
            ...
            C = self.cost_class * cost_class + self.cost_bbox * cost_bbox + self.cost_giou * cost_giou + self.cost_span * cost_span[:,[i]] + \
                self.cost_relevance * cost_valid + self.cost_mask * cost_mask + self.cost_dice * cost_dice  # [q, 1]
...
def build_matcher(args):
    ...
    return HungarianMatcher(cost_class=args.set_cost_class, 
                            ...
                            cost_relevance=args.set_cost_relevance,
