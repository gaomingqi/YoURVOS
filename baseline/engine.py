...
def train_one_epoch_mix(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, max_norm: float = 0):
    """
    Mix training in spatial-temporal domains
    2 stages:
    1: spatial infernece only (mask generation)
    2: spatial-temporal inference ()
    """
    model.train()
    criterion.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        samples = samples.to(device)
        captions = [t["caption"] for t in targets]
        mask_ids = [t["mask_id"] for t in targets]
        spans = [t["span"] for t in targets]

        targets = utils.targets_to(targets, device) 

        # parepare targets for spatial-temporal inference
        targets = [target | {'span': span, 'mask_id': mid} for target, span, mid in zip(targets, spans, mask_ids)]
        # spatial-temporal inference, predict logits / boxes / masks / spans in a single forward pass
        spatial_temporal_outputs = model(samples, captions, targets)
        # compute losses for logits / boxes / masks / spans
        loss_dict = criterion(spatial_temporal_outputs, targets)

        spatial_temporal_weight_dict = criterion.weight_dict
        losses = sum(loss_dict[k] * spatial_temporal_weight_dict[k] \
                                      for k in loss_dict.keys() if k in spatial_temporal_weight_dict)

        # reduce losses over all GPUs for logging purposes
        loss_dict_reduced = utils.reduce_dict(loss_dict)
        loss_dict_reduced_unscaled = {f'{k}_unscaled': v
                                      for k, v in loss_dict_reduced.items()}
        loss_dict_reduced_scaled = {k: v * spatial_temporal_weight_dict[k]
                                    for k, v in loss_dict_reduced.items() if k in spatial_temporal_weight_dict}
        losses_reduced_scaled = sum(loss_dict_reduced_scaled.values())
