...
def get_args_parser():
    parser = argparse.ArgumentParser('OMFormer training and inference scripts.', add_help=False)
    ...
    parser.add_argument('--set_cost_span', default=5, type=float,
                        help="Span coefficient in the matching cost")
    parser.add_argument('--set_cost_relevance', default=5, type=float,
                        help="Aux Span coefficient in the matching cost")
    ...
    parser.add_argument('--relevance_loss_coef', default=5, type=float)
    parser.add_argument('--irrelevance_coef', default=0.3, type=float,
                        help="Relative classification weight of the no-object class")
    parser.add_argument('--focal_alpha', default=0.25, type=float)

    # dataset parameters
    # ['yourvos', 'refcoco', 'refcoco+', 'refcocog', 'all']
    # 'all': using the three ref datasets for pretraining
    parser.add_argument('--dataset_file', default='yourvos', help='Dataset name') 
    ...
    parser.add_argument('--binary', default=True, action='store_false')
    parser.add_argument('--remove_difficult', action='store_true')

    # YoURVOS parameters
    parser.add_argument('--yourvos_path', type=str, default='[path to YoURVOS]')
    parser.add_argument('--num_frames_long', type=int, default=25)
    ...
    parser.add_argument('--ngpu', default=8, type=int, help='gpu number during inference')
    parser.add_argument('--split', default='test', type=str, choices=['valid', 'test'])
    parser.add_argument('--num_frames_global', default=24, type=int)
    parser.add_argument('--temporal_window_size', default=5, type=int)
