"""
Main-Training script of OMFormer
...
from engine import train_one_epoch, train_one_epoch_mix, evaluate, evaluate_a2d
...
def main(args):
    ...
    if args.dataset_file == 'yourvos':
        start_time = time.time()
        for epoch in range(args.start_epoch, args.epochs):
            if args.distributed:
                sampler_train.set_epoch(epoch)
            train_stats = train_one_epoch_mix(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('OMFormer training and evaluation script', parents=[opts.get_args_parser()])
